...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def _initialize_weights(self):
        ...
        weights["static_lr_embeddings"] = tf.Variable(
            tf.random_normal([self.field_sizes[0], 1], 0.0, 0.0001),
            name="static_lr_embeddings")
        weights["dynamic_lr_embeddings"] = tf.Variable(
            tf.random_normal([self.field_sizes[1], 1], 0.0, 0.0001),
            name="dynamic_lr_embeddings")
        if self.extern_lr_size:
            weights["extern_lr_embeddings"] = tf.Variable(
            tf.random_normal([self.extern_lr_size, 1], 0.0, 0.001),
            name="extern_lr_embeddings")

        # embeddings
        weights["static_ffm_embeddings"] = tf.Variable(
            tf.random_normal([self.total_feature_sizes[0], self.embedding_size * self.total_field_size], 0.0, 0.0001),
            name="static_ffm_embeddings")  # static_feature_size * [K * F]
        weights["dynamic_ffm_embeddings"] = tf.Variable(
            tf.random_normal([self.total_feature_sizes[1], self.embedding_size * self.total_field_size], 0.0, 0.0001),
    ...
    def fit(self, train_static_index, train_dynamic_index, train_dynamic_lengths, train_y,
            valid_static_index=None, valid_dynamic_index=None, valid_dynamic_lengths=None, valid_y=None, combine=False):
        """
