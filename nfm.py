...
class TextIterator:
    def __init__(self,hparams,mode,batch_size=None):
        ...
        if 'train' in mode:
            ...
            self.all_process=hparams.all_process
        else:
            self.idx=0
            self.all_process=1
...
class Model(object):
    def __init__(self,hparams):
        ...
        for s in hparams.single_features:
            ...
            if s in self.f1:
                self.emb_v2['f1_'+s]=tf.Variable(tf.truncated_normal(shape=[len(hparams.dict[s])+2,hparams.k], mean=0.0, stddev=0.0001),name='emb_v2_f1_'+s) 

            if s in self.f2:
                self.emb_v2['f2_'+s]=tf.Variable(tf.truncated_normal(shape=[len(hparams.dict[s])+2,hparams.k], mean=0.0, stddev=0.0001),name='emb_v2_f2_'+s)
    ...
    def build_graph(self, hparams):
        ...
        for s in hparams.mutil_features:
            emb_inp_v1[s]=tf.reduce_max(tf.gather(self.emb_v1[s], self.mulit_ids[s]),axis=1) 
            tf.add_to_collection('l2_loss',tf.nn.l2_loss(emb_inp_v1[s])*hparams.l2)
        for s in emb_inp_v1:
            if hparams.use_dropout:
                emb_inp_v1[s]=tf.cond(self.use_dropout, lambda: tf.nn.dropout(emb_inp_v1[s], 1-hparams.dropout), lambda: emb_inp_v1[s]) 
        ...
        for s in hparams.single_features:
            if s in self.f1:
                emb_inp_v2['f1_'+s]=tf.gather(self.emb_v2['f1_'+s], self.single_ids[s])
                tf.add_to_collection('l2_loss',tf.nn.l2_loss(emb_inp_v2['f1_'+s])*hparams.l2)
            if s in self.f2:
                emb_inp_v2['f2_'+s]=tf.gather(self.emb_v2['f2_'+s], self.single_ids[s])
                tf.add_to_collection('l2_loss',tf.nn.l2_loss(emb_inp_v2['f2_'+s])*hparams.l2)

            
        
        for s in hparams.mutil_features:
            if s in self.f1:
                emb_inp_v2['f1_'+s]=tf.reduce_max(tf.gather(self.emb_v2['f1_'+s], self.mulit_ids[s]),axis=1)
                tf.add_to_collection('l2_loss',tf.nn.l2_loss(emb_inp_v2['f1_'+s])*hparams.l2)
            if s in self.f2:
                emb_inp_v2['f2_'+s]=tf.reduce_max(tf.gather(self.emb_v2['f2_'+s], self.mulit_ids[s]),axis=1)
                tf.add_to_collection('l2_loss',tf.nn.l2_loss(emb_inp_v2['f2_'+s])*hparams.l2)
        ...
        for s in self.f1:
            x[0].append(emb_inp_v2['f1_'+s][:,None,:])
        for s in self.f2:
            x[1].append(emb_inp_v2['f2_'+s][:,None,:])
        x[0]=tf.concat(x[0],1)
        x[1]=tf.concat(x[1],1)

        emb_rep_v_1=x[0][:,:,None,:]*x[1][:,None,:,:]
        emb_rep_v_1=tf.reshape(emb_rep_v_1,[-1,len(self.f1)*len(self.f2),hparams.k])

        


        
        emb_rep_v=tf.concat([emb_rep_v_1],1)
        w2=tf.reduce_sum(emb_rep_v,[-1,-2])

        emb_rep_v=tf.concat([tf.reduce_max(emb_rep_v,-1),tf.reduce_mean(emb_rep_v,-1)],-1)

        temp=[]
        temp.append(emb_rep_v)
        for s in emb_inp_v2:
            temp.append(emb_inp_v2[s])

        emb_rep_v=tf.concat(temp,-1)
        ...
        W_1 = tf.get_variable("W_1", [emb_rep.get_shape()[-1],hparams.hidden_size*4],initializer=initializer)
        b_1 = tf.get_variable("b_1", [hparams.hidden_size*4],initializer=initializer) 

        tf.add_to_collection('mlp_loss',tf.nn.l2_loss(W_1)*hparams.l2)
        tf.add_to_collection('mlp_loss',tf.nn.l2_loss(b_1)*hparams.l2)

        hidden_outputs_1=tf.nn.relu(tf.tensordot(emb_rep,W_1,[[-1],[0]])+b_1) 
        hidden_outputs_1=self.batch_norm_layer(hidden_outputs_1,self.use_dropout,'train_1')




        W_2 = tf.get_variable("W_2", [hidden_outputs_1.get_shape()[-1],hparams.hidden_size*2],initializer=initializer)
        b_2 = tf.get_variable("b_2", [hparams.hidden_size*2],initializer=initializer)    
        tf.add_to_collection('mlp_loss',tf.nn.l2_loss(W_2)*hparams.l2)
        tf.add_to_collection('mlp_loss',tf.nn.l2_loss(b_2)*hparams.l2)
        hidden_outputs_2=tf.nn.relu(tf.tensordot(hidden_outputs_1,W_2,[[-1],[0]])+b_2)


        
        W_3 = tf.get_variable("W_3", [hidden_outputs_2.get_shape()[-1]],initializer=initializer)
        b_3 = tf.get_variable("b_3", [1],initializer=initializer)
        w_3=tf.tensordot(hidden_outputs_2,W_3,[[-1],[0]])+b_3  

        tf.add_to_collection('mlp_loss',tf.nn.l2_loss(W_3)*hparams.l2)
        tf.add_to_collection('mlp_loss',tf.nn.l2_loss(b_3)*hparams.l2)
        ...
        self.cost=-tf.reduce_mean(hparams.weight*self.label*logit_1+(1-self.label)*logit_0)
    ...
    def batch_norm_layer(self, x, train_phase,scope_bn):
        ...
        z = tf.cond(train_phase, lambda: bn_train, lambda: bn_train)
        return z
def train(hparams):
            ...
            while True:
                    ...
                    if global_step%hparams.num_display_steps==0:
                            ...
                            if global_step%hparams.num_eval_steps==0 or epoch:
                                ...
                                loss_=np.mean(auc)
                                if best_loss<=loss_:
                                    model.saver_ffm.save(sess,os.path.join(hparams.path, 'model_nfm'+str(hparams.idx)))
                                    best_loss=loss_
                                ...
                                if epoch_cont==hparams.epoch:
                                    model.saver_ffm.restore(sess,os.path.join(hparams.path, 'model_nfm'+str(hparams.idx)))
                                    break
        print("Dev inference ...")
        preds=[]
        label=[]
        ...
        df.to_csv('result_logloss/nfm_submission_dev'+str(hparams.idx)+'.csv',index=False)
